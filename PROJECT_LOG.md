...
- **H2 (PII in public mandate field) — DONE 14 Jul:** `mandates.talent_pool_cache` + `linkedin_search_cache` (candidate names/scores/LinkedIn profiles) were exposed because the public job detail page did `select("*")` on active mandates. Fixed: (1) job page now selects only safe columns; (2) `REVOKE SELECT (talent_pool_cache, talent_pool_cached_at, linkedin_search_cache) ON mandates FROM anon` (defense in depth). Verified public jobs list + detail render fine; staff internal page keeps cache access (authenticated, unaffected). RESIDUAL NOW CLOSED (15 Jul): moved both caches to a new staff-only table **`mandate_ai_cache`** (RLS: only active `staff_users` by email can read/write; anon revoked). Migrated all 18 cached rows, repointed the staff mandate page to read/write the new table, then **dropped `talent_pool_cache` / `talent_pool_cached_at` / `linkedin_search_cache` from `mandates`** entirely. Verified live: staff sees cached Talent Pool + LinkedIn results (read OK) and can write (RLS update = 1 row); a simulated non-staff authenticated user sees 0 rows and writes 0 (blocked). App unaffected after column drop. H2 = fully done.
