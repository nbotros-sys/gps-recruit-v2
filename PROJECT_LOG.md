...
- **H2 (PII in public mandate field) — DONE 14 Jul:** `mandates.talent_pool_cache` + `linkedin_search_cache` (candidate names/scores/LinkedIn profiles) were exposed because the public job detail page did `select("*")` on active mandates. Fixed: (1) job page now selects only safe columns; (2) `REVOKE SELECT (talent_pool_cache, talent_pool_cached_at, linkedin_search_cache) ON mandates FROM anon` (defense in depth). Verified public jobs list + detail render fine; staff internal page keeps cache access (authenticated, unaffected). Residual (narrow): a logged-in non-staff could still direct-query these — full close = move cache to a staff-only table (follow-up).
