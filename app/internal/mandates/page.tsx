...
import { Plus, Search, Filter, Briefcase, MapPin, DollarSign, Calendar, ChevronRight, X } from "lucide-react"
...
export default function MandatesPage() {
  ...
  async function createMandate(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("mandates").insert([{
      title: form.title, client_name: form.client_name, location: form.location,
      salary_range: form.salary_range, job_description: form.job_description, status: form.status
    }])
    if (!error) {
      setShowCreate(false)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{mandates.length} total mandates</p>
        </div>
      ...
      <div className="card p-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search mandates by title or client..."
      ...
      {/* List */}
      {loading ? (
        <div className="text-center py-16 text-gray-400">Loading mandates...</div>
      ) : filtered.length === 0 ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 flex items-center gap-3 mt-0.5">
                    ...
                    {m.salary_range && <span className="flex items-center gap-1"><DollarSign size={12} />{m.salary_range}</span>}
                  </div>
                </div>
              </div>
            ...
            <form onSubmit={createMandate} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea required value={form.job_description} onChange={e => setForm({...form, job_description: e.target.value})}
                  rows={6}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal/30 focus:border-teal resize-none"
                  placeholder="Paste the full job description here. This will be used by AI to score and match candidates automatically." />
              </div>
