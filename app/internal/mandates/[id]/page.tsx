...
export default function MandateDetail() {
  ...
  async function runBulkScore() {
    ...
    for (let i = 0; i < files.length; i++) {
      setBulkStatus(`Reading CV ${i + 1} of ${files.length}: ${files[i].name}`)
    ...
    setBulkStatus(`AI scoring all ${files.length} candidates...`)
    setBulkProgress(65)
    try {
    ...
    try {
      ...
      const { data: candidate, error: candError } = await supabase
        ...
        .insert([{
          ...
          email: `${r.name.toLowerCase().replace(/\s+/g, ".")}@pending.com`,
          source: "direct",
          tags: [],
      ...
      // Create application linking candidate to mandate
  ...
  async function addAllProceed() {
    setAddingAll(true)
    const proceedIndexes = bulkResults
      .map((r, i) => ({ r, i }))
      .filter(({ r }) => r.recommendation === "Proceed" && !r.added)
      .map(({ i }) => i)

    for (const idx of proceedIndexes) {
      await addToPipeline(idx)
    }
    setAddingAll(false)
  }
          ...
          {bulkResults.length === 0 && (
            <>
              ...
              >
                ...
                <p className="text-gray-300 text-xs mt-3">PDF, Word (.docx), or text files · Multiple files at once</p>
              </div>
                ...
                <div className="card p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <span className="text-gray-400 text-sm ml-2">→ {mandate.title}</span>
                    </div>
                  ...
                  {bulkProcessing ? (
                    <div className="space-y-3">
                      ...
                      <p className="text-xs text-gray-400 text-center">{bulkProgress}% — may take a minute for large files</p>
                    </div>
                  ) : (
                    <button onClick={runBulkScore} className="btn-primary w-full py-3 flex items-center justify-center gap-2 text-base">
          ...
          {bulkResults.length > 0 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <h3 className="font-semibold text-gray-900">Ranked by fit score</h3>
                  {addedCount > 0 && (
                    <span className="badge bg-teal/10 text-teal">{addedCount} added to pipeline</span>
                  )}
                </div>
                  ...
                  {proceed > 0 && bulkResults.filter(r => r.recommendation === "Proceed" && !r.added).length > 0 && (
                    <button onClick={addAllProceed} disabled={addingAll}
                      ...
                      {addingAll
                        ...
                        : <><Users size={13} /> Add all {proceed} "Proceed" to pipeline</>
                      }
                    </button>
                  )}
              ...
              {bulkResults.map((r, i) => (
                <div key={i}
                  className={`bg-white rounded-2xl border overflow-hidden transition-all
                    ${r.added ? "border-teal/30 bg-teal/[0.02]" : expandedCard === i ? "border-teal/20 shadow-md" : "border-gray-100 shadow-sm hover:shadow-md"}`}
                >
                  <div className="p-5">
                    <div className="flex items-center gap-4">
                      ...
                      {/* Recommendation */}
                      <span className={`badge flex-shrink-0 text-xs font-semibold px-3 py-1
                      ...
                      {r.added ? (
                        <span className="flex items-center gap-1.5 text-teal text-sm font-medium flex-shrink-0">
                          <CheckCircle size={15} /> In pipeline
                        </span>
                      ) : (
                        <button onClick={() => addToPipeline(i)} disabled={r.adding}
                          className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-teal/30 text-teal text-sm font-medium hover:bg-teal/5 transition-all disabled:opacity-50">
                          {r.adding ? <Loader2 size={13} className="animate-spin" /> : <UserPlus size={13} />}
                      ...
                      <button onClick={() => setExpandedCard(expandedCard === i ? null : i)}
                        className="text-gray-300 hover:text-gray-500 flex-shrink-0 transition-transform duration-200"
                        ...
                        {r.strengths?.length > 0 && (
                          <div className="bg-green-50 rounded-xl p-4">
                            ...
                            <ul className="space-y-1.5">
                              {r.strengths.map((s, j) => <li key={j} className="text-xs text-green-800 flex gap-2"><span>•</span>{s}</li>)}
                            </ul>
                          </div>
                        )}
                        {r.concerns?.length > 0 && (
                          <div className="bg-amber-50 rounded-xl p-4">
                            ...
                            <ul className="space-y-1.5">
                              {r.concerns.map((c, j) => <li key={j} className="text-xs text-amber-800 flex gap-2"><span>•</span>{c}</li>)}
                            </ul>
                          </div>
                        )}
                      </div>
